...
from tensorflow.keras.layers import Input, Dense, LSTM, GRU, Dropout, concatenate, Bidirectional
...
samsung_csv = pd.read_csv(path + 'samsung_data.csv',
                          index_col=0, header=0, encoding='cp949', thousands=',', nrows=1166)
amore_csv = pd.read_csv(path + 'amore_data.csv',
                        index_col=0, header=0, encoding='cp949', thousands=',', nrows=1166)
"""
...
lstm21 = LSTM(50, activation='relu', name='dns21')(input2)
...
model.fit([sam_x_train, amo_x_train], sam_y_train, epochs=1000, batch_size=2,
          validation_split=0.1, callbacks=[es, mcp], verbose=3)


##### 저장한 모델 불러오기 #####
# model = load_model('save_MCP/samsung_0129_1806_195-424582.3750.hdf5')
...
r2 = r2_score(sam_y, result)
def RMSE(y_test, y_predict):
    return np.sqrt(mean_squared_error(y_test, y_predict))

print("\nloss:", loss)
print("R2:", r2)
print("RMSE:", RMSE(sam_y, result), "\n")

for i in range(4, 1, -1):
  i *= -1
  print(f'2023.01.2{i + 9} 실제 시가: {sam_y[i]} / 예측 시가: {result[i]}')
...
< 결과 >
loss: 776019.4375
R2: 0.9968764694393776
RMSE: 750.5999407720747
2023.01.25 실제 시가: [62100] / 예측 시가: [61061.023]
2023.01.26 실제 시가: [63500] / 예측 시가: [61947.14]
2023.01.27 실제 시가: [63800] / 예측 시가: [63459.656]
2023.01.30 삼성전자 예측 시가: [63815.938]
"""